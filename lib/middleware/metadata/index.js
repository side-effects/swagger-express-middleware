...
function metadataMiddleware (context, router) {
  ...
  function setApi (req, res, next) {
    ...
    if (context.api) {
      ...
      let reqPath = util.normalizePath(req.baseUrl, router);
      reqPath = reqPath + util.normalizePath(req.path, router)
  ...
  function setPath (req, res, next) {
    if (req.openapi.api) {
      let reqUrl = getRelativePath(req, router);
 ...
 * @returns {string}
 */
function getRelativePath (req, router) {
  ...
  if (!basePath) {
    ...
    let p = util.normalizePath(req.baseUrl, router);
    p = p + req.path;
    return p.substr(basePath.length);
  }
}
